...
from src.combat.combat_basics import dodge_dice_roll
...
def test_attack_dodged(mock_dice):
    ...
    assert dodge_dice_roll(accuracy=5, dodge=3) is False

    # Test case where dice_dodge is 8 (always hits)
    mock_dice.side_effect = [8]
    assert dodge_dice_roll(accuracy=5, dodge=3) is True
    ...
    mock_dice.side_effect = [4, 3]
    assert dodge_dice_roll(accuracy=5, dodge=6) is True
