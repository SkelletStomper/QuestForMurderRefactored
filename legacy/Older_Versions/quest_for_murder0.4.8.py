#Quest for Murder
#A text RPG by Ole Haufschild
#To Do:
#-Quest log (done)-Quests get removed after completion
#-Armmor and Weapons
#-Special abilities
#-Gold and Merchants
#-Portal Gun must do stuff
import random
patchesdead = False
gamefinished = False
true = True
...
while not gamefinished:
    ...
    #weapons
    weaponid = 0
    weaponname = ["bare fist","longsword","curved dagger","mace","spoon","spear","heavy warhammer"]
    weapondmg = [0,3,2,2,0,3,5]
    weaponamr = [0,0,0,0,0,0,1]
    weaponweight = [0,3,1,2,0,3,7]
    weaponclass = ["fist","sword","dagger","mace","joke","spear","mace"]
    weaponstartattribute = ["none","none","none","blessed","mirror","none","heavy"]
    weaponattribute = weaponstartattribute[weaponid]
    weaponsmithed = "classic"

    #offhand weapons
    offhandid = 0
    offhandname = ["bare fist","knight shield","curved dagger"]
    offhanddmg = [0,0,2]
    offhandamr = [0,2,0]
    offhandheal = [0,0,0]
    offhandweight = [0,2,1]
    offhandclass = ["fist","shield","dagger"]
    offhandstartattribute = ["none","none","none"]
    offhandattribute = offhandstartattribute[offhandid]
    offhandsmithed = "classic"
    
    #armor
    armorid = 0
    armorname = ["normal clothing","plate armor","smooth leather armor","dirty rags"]
    armoramr = [0,3,1,0]
    armorweight = [0,4,1,0]
    armorstartattribute = ["none","heavy","none","stenching"]
    ...
    if name == "":
        ...
        print("\""+ name + ", you say? Well, it does not really matters to me. But I have some good advice for you. You have chosen a bad time to wake up from your slumber- Its the evening of bloodlust!")
    ...
    while not rolechosen2:
        ...
        while not rolechosen:
            ...
            if role == "knight":
                ...
                roledescription = "A filthy cleric, Patches does not like him. He has extremely high Heal, and a bit armored. He starts with a mace, which is blessed."
                maxhp = 14
                basedmg = 2
                weaponid = 3
                offhandid = 0
    ...
    while stage < 6 and not gamefinished:
        ...
        if not stage == 5:
            if attr > 50 and attr < 100:
                ...
                hpemax += -stage*5
                if hpemax > 1:
                    print(titleBig + adjective + mnsrtp + " was so weak that it failed to live.")
        ...
        while hpe > 0 and hp > 0 and not gamefinished:
            ...
            while not done and not gamefinished and hp > 0:
                ...
                if do == "atk":
                    dmg = int(basedmg + weapondmg[weaponid] + offhanddmg[offhandid])
                    ...
                    if hpe < 1:
                        ...
                        if randomnumb == 1:
                            ...
                            print("You attacked for " + str(int((dmg)*acc)) + " damage, and destroyed an important organ." + title + mnstrtp + " falls backwards and does not get up again.")
                        else:
                            print("You attacked for " + str(int((dmg)*acc)) + " damage, which was a lethal hit!")
                    else:
                    ...
                    if mnstrtp == "devil-possessed baby":
                        ...
                        dodge = dodge - 1
                        crtmdfe = crtmdfe + 1
                        print("As you hit " + title + mnstrtp + ", it explodes and coveres you with its organs! You have " + str(hp) + " Hp left and your Dex decreased!")
                        death_message = "After this heavy explosion, all that is left of you are some pieces, spreaded in a 10m radius. You are dead."
                        done = True
                    elif mnstrtp == "literal devil":
                        hpe += dmg + weapondmg + offhanddmg
                        if hpe > hpemax:
                            hpe = hpemax
                    ...
                    invdo = input("(1) Healing potion(" + str(healpots) + ")  (2) Grenade(" + str(dmgpots) + ") (3) I want to see my stuff!  (4) Exit(" + str(exittimes) + ")")
                    if invdo == "1":
                        if healpots > 0:
                            hp += int(heal*2.5)
                            healpots += -1
            ...
            if hpe > 0 and not gamefinished:
                ...
                while etimesattacked < eturns:
                    if dodge + random.randint(0,10) > 9:
                        ...
                        if acce > 1.6 + stage:
                            acce = 1.6 + stage
                        if int(((dmge) * acce - armor - armoramr[armorid])) > 0:
                            hp = hp - int(((dmge) * acce - armor - armoramr[armorid]))
                        if int(((dmge) * acce - armor)) > 0:
                            print(titleBig + mnstrtp + " attacked you for " + str(int(((dmge) * acce - armor-armoramr[armorid]))) + " damage! You have " + str(hp) + " HP left!")
                        else:
            ...
            if not gamefinished:
                ...
                while lvlupd < 1 and not gamefinished:
                    ...
                    #LOOT!
                    randomnumb = random.randint(0,len(lootlist))
                    loot = lootlist[randomnumb]
                    if loot != "w-0-classic":
                        loot = loot.split("-")
                        loottype = loot[0]
                        ...
                        if loottype == "w":
                            ...
                            if lootsmithed == "classic":
                                if offhandstartttribute[lootid] == "none":
                                    print("It has no special modifier on it.")
                                else:
